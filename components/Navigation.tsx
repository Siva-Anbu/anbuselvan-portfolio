...
export default function Navigation() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a] border-b border-white/5">
        <nav className="max-w-[1600px] mx-auto px-6 md:px-12 h-16 md:h-20 flex items-center justify-between">
          ...
          <Link href="/" className="group flex items-baseline gap-2 leading-none">
            <span
              className="font-display text-xl md:text-2xl font-light tracking-[0.08em] uppercase text-white/90 group-hover:text-white transition-colors"
            >
              Anbuselvan
            </span>
            <span
              className="font-display text-xl md:text-2xl font-light tracking-[0.08em] uppercase"
      ...
      {/* Mobile menu overlay */}
      <div
        className={`fixed inset-0 z-40 bg-[#0a0a0a] flex flex-col justify-center px-12 transition-all duration-500 md:hidden ${
          menuOpen
      ...
      >
        <ul className="flex flex-col gap-8">
          {navLinks.map(({ href, label }, i) => (
            ...
            >
              <Link
                href={href}
                className={`font-display text-4xl font-light transition-all duration-500 block ${
                  menuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                } ${pathname === href ? '' : 'text-white/60 hover:text-white'}`}
        ...
        <div
          className="absolute bottom-12 left-12 font-display text-sm font-light text-white/20"
        >
